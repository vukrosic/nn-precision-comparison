...
print(f"FP8 - Using device: {device}")
...
for epoch in range(epochs):
    ...
    if epoch % save_every == 0:
        with torch.no_grad():
            ...
            ax.plot(x_cpu, y_pred_cpu, 'r--', linewidth=2, label='FP8')
            ax.set_xlim(-2, 2)
            ax.set_ylim(-5, 5)
            ax.set_title(f'FP8 - Epoch {epoch}, Loss: {loss.item():.4f}')
            ax.legend()
...
print(f"FP8 complete! Final loss: {loss.item():.6f}")